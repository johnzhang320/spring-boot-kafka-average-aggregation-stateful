...
public class MovieController {
    ...
    public CountSumAverage retriveMovieStore(@PathVariable("movieId") Long movieId) {
        CountSumAverage countAndSum = retrieveRatingAverageService.getStoreCountAndSum(movieId);
        log.info(countAndSum.toString());
        return countAndSum;
    }
    // http://localhost:8098/movie/getAllAverageRating
    @GetMapping("/getAllAverageRating")
    public List<KeyValue<Long, CountSumAverage>> retriveAllMovieAverageStore() {
